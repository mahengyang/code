...
do
  if [ -f "$desdir/$dirlist/test.env" ] ; then
    testenv=`cat "$desdir/$dirlist/test.env" | grep -E "user:|loop:|duration:|thinktime:|maxuser:"`
    api=`cat "$desdir/$dirlist/test.env" | grep api:`
    method=`cat "$desdir/$dirlist/test.env" | grep method:`
    #api contains ?
    api=${api%%\?*}
    echo "<a href=\"log/$dirlist\">$dirlist -- $method $api -- `echo $testenv | xargs`</a>"
    echo "<br>"
